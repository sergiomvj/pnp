-- ========================================
-- SCHEMA FINAL - PULSE & PERSPECTIVE
-- ========================================
-- Versão sem inconsistências, completamente funcional
...
-- Adiciona a coluna 'completed' se ela não existir, para garantir a retrocompatibilidade
DO $$
BEGIN
  IF NOT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='reading_history' AND column_name='completed') THEN
    ALTER TABLE reading_history ADD COLUMN completed BOOLEAN DEFAULT false;
  END IF;
END $$;
...
-- Categories e Tags (público para leitura)
...
DROP POLICY IF EXISTS "Tags viewable by everyone" ON tags;
...
-- Article Likes
DROP POLICY IF EXISTS "Users manage own likes" ON article_likes;
...
-- Social Shares
DROP POLICY IF EXISTS "Users manage own shares" ON social_shares;
...
-- Helena
DROP POLICY IF EXISTS "Users manage own conversations" ON helena_conversations;
CREATE POLICY "Users manage own conversations" ON helena_conversations 
  FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users view own messages" ON helena_messages;
CREATE POLICY "Users view own messages" ON helena_messages 
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM helena_conversations 
            WHERE helena_conversations.id = helena_messages.conversation_id 
            AND helena_conversations.user_id = auth.uid())
  );

DROP POLICY IF EXISTS "Users insert own messages" ON helena_messages;
CREATE POLICY "Users insert own messages" ON helena_messages 
